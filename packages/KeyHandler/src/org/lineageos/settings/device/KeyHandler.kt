...
class KeyHandler(private val context: Context) : DeviceKeyHandler {
    ...
    private val screenshotHelper by lazy { ScreenshotHelper(context) }
    ...
    override fun handleKeyEvent(event: KeyEvent): KeyEvent? {
        // The Plus Key needs both the press and the release to tell a short press from a
        // long one, so it is routed before the ACTION_DOWN filter below - and before the
        // alert-slider device gate, which names hardware this generation does not have.
    ...
    private fun populateKeyState(firstRun: Boolean) {
        val node = File(TRI_STATE_NODE)
        if (!node.exists()) {
            // No three-position slider on this generation; nothing to restore. Without
            // this the ACTION_BOOT_COMPLETED path throws on every boot.
            return
        }
    ...
    private fun applyPlusKeyAction(action: Int) {
        when (action) {
            ...
            PLUS_ACTION_SCREENSHOT ->
                // The platform helper drives SystemUI's capture path, which is what the
                // power+volume chord uses; there is nothing to inject.
            ...
            PLUS_ACTION_OPEN_APP -> {
                val packageName = sharedPreferences.getString(PLUS_KEY_APP, null)
                val intent =
                    packageName?.let { context.packageManager.getLaunchIntentForPackage(it) }
                if (intent != null) {
                    startActivitySafely(intent)
                } else {
                    // The picker only offers launchable packages, so this means the
                    // chosen app was uninstalled since.
                    Log.w(TAG, "no launcher intent for selected app $packageName")
                }
            }
        }
    }
    ...
    private fun applyAction(mode: Int, muteMedia: Boolean) {
        when (mode) {
            ...
            TORCH_ON,
            TORCH_OFF -> flashCameraId?.let {
                cameraManager.setTorchMode(it, mode == TORCH_ON)
            }
        }
    }
    ...
    companion object {
        ...
        // Alert slider state node. Absent on generations that ship the Plus Key
        // instead, which is why every read of it is guarded.
        private const val TRI_STATE_NODE = "/proc/tristatekey/tri_state"
        ...
        const val PLUS_KEY_APP = "config_plus_key_app"
        const val ACTION_NONE = -1

        // Plus Key action vocabulary. Its own numbering, stored under its own
        // preference keys and dispatched by applyPlusKeyAction alone, so it never
        // meets the slider's ringer-position values in the same when.
        const val PLUS_ACTION_ASSISTANT = 1
        private const val PLUS_ACTION_CAMERA = 2
        private const val PLUS_ACTION_SCREENSHOT = 3
        ...
        private const val PLUS_ACTION_OPEN_APP = 10
